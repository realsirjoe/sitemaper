...
func Run(ctx context.Context, args []string, stdout, stderr io.Writer, cfg Config) int {
	...
	fs.Usage = func() {
		...
		fmt.Fprintln(stderr)
		fmt.Fprintln(stderr, "Query syntax:")
		fmt.Fprintln(stderr, "  Think of the sitemap tree like a Unix filesystem.")
		fmt.Fprintln(stderr, "  Use :: as the path separator (like /).")
		fmt.Fprintln(stderr, "  Example: --query ::            (root)")
		fmt.Fprintln(stderr, "           --query foo.xml       (child of root)")
		fmt.Fprintln(stderr, "           --query foo.xml::bar  (grandchild)")
		fmt.Fprintln(stderr)
